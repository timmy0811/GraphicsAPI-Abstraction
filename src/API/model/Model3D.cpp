...
void API::Model::Model3D::ProcessNode(const aiNode* node, const aiScene* scene)
{
...
std::vector<API::Texture::Texture*> API::Model::Model3D::LoadMaterialTextures(const aiMaterial* mat, const aiTextureType type)
{
	std::vector<Texture::Texture*> textures;
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{
			if (std::strcmp(m_LoadedTexture->GetPath().data(), path.c_str()) == 0)
			{
		...
		if (!skip)
		{
			Texture::Texture* texture = Texture::Texture::Create(path, Texture::TextureFilter::LINEAR, false); // m_Directory + str ???
...
{
	for (const Texture::Texture* tex : m_LoadedTextures)
	{
		delete tex;
	}
}

void API::Model::Model3D::Draw(Core::Shader& shader)
{
