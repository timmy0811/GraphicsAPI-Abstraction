...
namespace API::Misc
{
	struct Sprite
	{
		...
		Sprite(const std::string& path, const glm::vec2& position, const glm::vec2& size, Vec2_4 uvs,
	...
	struct SpriteBlueprint : public Sprite
	{
		...
		SpriteBlueprint(const std::string& path, const glm::vec2& position, const glm::vec2& size,
		                const bool flipUV = false) :
			Sprite(path, position, size, flipUV), vertices{}
		{
		}

		SpriteBlueprint(const std::string& path, const glm::vec2& position, const glm::vec2& size, Vec2_4 uvs,
		                const bool flipUV = false) :
			Sprite(path, position, size, uvs, flipUV), vertices{}
		{
		}
	};
	...
	{
	public:
		SpriteRenderer(int maxSprites, glm::uvec2 samplerSlotRange, int slotDefault, glm::vec2 winSize,
		               const std::string& shaderVert, const std::string& shaderFrag);
		~SpriteRenderer();
		...
		unsigned int PushSprite(const std::string& path, const glm::vec2& position, const glm::vec2& size,
		                        const Vec2_4& uvs, const bool flipUV = false);
		unsigned int PushSprite(const std::string& path, const glm::vec2& position, const glm::vec2& size,
		                        bool flipUV = false);

		void TransformSprite(const Vec2_4& uvs, unsigned int id);
	...
	private:
		std::shared_ptr<Core::VertexBuffer> m_VB;
		std::shared_ptr<Core::IndexBuffer> m_IB;
		std::shared_ptr<Core::VertexBufferLayout> m_VBLayout;
		std::shared_ptr<Core::VertexArray> m_VA;
