...
namespace API::Advanced
{
	...
	public:
		...
		virtual void BindTextures(unsigned int startSlot) = 0;
		...
		virtual std::string GetTargetIdentifier(unsigned int internalId) = 0;

		virtual void BindDepthTexture(unsigned int slot) = 0;

		virtual unsigned int AddRenderTarget(const std::string& identifier, unsigned int width, unsigned int height, unsigned int components, Core::BufferDataType datatype,
		                                     Core::WrapMethod wrap = Core::WrapMethod::CLAMP_TO_EDGE, void* data = nullptr) = 0;
		virtual unsigned int AddRenderTarget(const std::string& identifier, unsigned int components, Core::BufferDataType datatype,
		                                     Core::WrapMethod wrap = Core::WrapMethod::CLAMP_TO_EDGE, void* data = nullptr) = 0;
